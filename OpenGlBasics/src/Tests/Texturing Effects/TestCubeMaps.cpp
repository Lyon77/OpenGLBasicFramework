	...
	void TestCubeMaps::OnRender()
	{
		...
		// Load the box
		{
			...
			else
			{
				...
				m_RefractiveShader->SetUniform3f("u_CameraPos", Camera::GetInstance().CameraPosition().x, Camera::GetInstance().CameraPosition().y, Camera::GetInstance().CameraPosition().z);
