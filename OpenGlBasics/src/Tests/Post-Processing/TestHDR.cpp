...
namespace test {
		...
		m_Exposure(1.0f)
	{
		...
		m_VertexBuffer = std::make_unique<VertexBuffer>(positions, 8 * 4 * 1 * sizeof(float)); //3 values, 4, points, 1 face
	...
	void TestHDR::OnRender()
	{
		...
		// Load the box
		{
			...
			m_Shader->SetUniform3f("u_PointLight[1].position", m_LampPos.x + 0.1f, m_LampPos.y -0.4f, m_LampPos.z + 3.0f);
			m_Shader->SetUniform3f("u_PointLight[1].ambient",  0.0f, 0.0f, 0.2f);
			m_Shader->SetUniform3f("u_PointLight[1].diffuse",  0.0f, 0.0f, 0.2f);
			m_Shader->SetUniform3f("u_PointLight[1].specular", 0.0f, 0.0f, 0.2f);
			...
			m_Shader->SetUniform3f("u_PointLight[2].position", m_LampPos.x - 0.4f, m_LampPos.y - 0.1f, m_LampPos.z + 3.0f);
			m_Shader->SetUniform3f("u_PointLight[2].ambient",  0.2f, 0.0f, 0.0f);
			m_Shader->SetUniform3f("u_PointLight[2].diffuse",  0.2f, 0.0f, 0.0f);
			m_Shader->SetUniform3f("u_PointLight[2].specular", 0.2f, 0.0f, 0.0f);
		...
		// Load the Lamp
		{
...

			{
				...
				m_LampShader->SetUniform4f("u_Color", 3.0f, 3.0f, 3.0f, 1.0f);
...

			{
				...
				m_LampShader->SetUniformMat4f("u_MVP", mvp);
				m_LampShader->SetUniform4f("u_Color", 0.0f, 0.0f, 0.2f, 1.0f);
...

			{
				...
				m_LampShader->SetUniformMat4f("u_MVP", mvp);
				m_LampShader->SetUniform4f("u_Color", 0.2f, 0.0f, 0.0f, 1.0f);
	...
	void TestHDR::MouseCallback(GLFWwindow* window, double xpos, double ypos)
	{
	...
	{
		ImGui::TextWrapped("Welcome to the HDR Test Enviroment. Use WASD to move around and QE to zoom in and out. There are more setting options below.");
